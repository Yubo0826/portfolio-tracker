...
export const usePortfolioStore = defineStore('portfolio', () => {
    ...
    async function fetchPortfolios() {
        ...
        try {
            const data: any = await api.get(`/api/portfolio?uid=${auth.user.uid}`);
            console.log('Fetched portfolios:', data);
            portfolios.value = data.portfolios;
            if (data.portfolios.length > 0) {
                const localStoragePortfolioStr = localStorage.getItem('currentPortfolio');
                const localStoragePortfolio = localStoragePortfolioStr ? JSON.parse(localStoragePortfolioStr) : null;
    ...
    function setPortfolios(portfoliosList: Portfolio[]) {
    ...
    // 設定目前使用的投資組合
    function setCurrentPortfolio(portfolio: Portfolio | null) {
    ...
    // 之後後端or前端可能要卡重複名稱
    async function addPortfolio(newPortfolio: Partial<Portfolio>) {
        ...
        try {
            const data: any = await api.post('/api/portfolio', {
                uid: auth.user.uid,
                name,
                description,
                drift_threshold,
    ...
    async function editPortfolio(portfolioId: string, updatedPortfolio: Partial<Portfolio>) {
        ...
        try {
            const data: any = await api.put(`/api/portfolio/`, {
                uid: auth.user.uid,
                id: portfolioId,
                name,
                description,
    ...
    async function removePortfolio(ids: string[]) {
